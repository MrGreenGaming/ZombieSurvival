...
if CLIENT then		
	...
	killicon.AddFont("weapon_zs_pulse_railgun", "HL2MPTypeDeath", SWEP.IconLetter, Color(100, 255, 255, 255 ))		
...
SWEP.Pulse_Static_RechargeRate = 0.06
SWEP.Pulse_Static_ClipSize = 100
SWEP.Primary.BulletsUsed = 100

local function DoRailExplosion(attacker, hitpos, hitnormal, normal, damage)
	...
	explode:SetOwner( attacker ) -- this sets you as the person who made the explosion
	explode:Spawn() --this actually spawns the explosion
	explode:SetKeyValue( "iMagnitude", damage ) -- the magnitude
	...
	shake:Spawn()
	shake:SetOwner( attacker )
	shake:Activate()
	shake:Fire( "StartShake", "", 0 )	
end

function SWEP.BulletCallback(attacker, tr, dmginfo)
	if SERVER and tr.Hit and not tr.HitSky then
		local hitpos, hitnormal, normal, dmg = tr.HitPos, tr.HitNormal, tr.Normal, dmginfo:GetDamage()
		timer.Simple(0, function() DoRailExplosion(attacker, hitpos, hitnormal, normal, dmg) end)
	end
