...
function hud.DrawWeaponLabels()
    ...
    for k, ent in pairs(cachedEnts) do
        ...
        draw.SimpleTextOutlined( name, "ArialBoldFive", 0, 0, Color(255,255,255,255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT, 2, Color(0,0,0,255) ) 
        cam.End3D2D()
    end
end
...
function hud.DrawAmmoPanel()
	...
	if PrimaryAmmo ~= -1 then
		...
		if SecondaryAmmo > 0 then
			...
			draw.SimpleTextOutlined("/", "NewAmmoFont13", xpos+w/2.5, y+h/2, Color(255,255,255,255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER,1,Color(0,0,0,255))
			...
			draw.SimpleTextOutlined(SecondaryAmmo, "NewAmmoFont13", xpos+w/2.5, y+h/2, Color(255,255,255,255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER,1,Color(0,0,0,255))
		end
	end
end
...
function hud.DrawHealthPanel()
	...
	draw.SimpleTextOutlined(math.Round(fHealth), "NewAmmoFont21", ActualX, ActualY, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,1,Color(0,0,0,255))
	...
	--Color of healthbar
	--local fHealth, sHealthIndication = COLOR_HUD_HEALTHY
	--if 0.8 < iPercentage then
	--	fHealth = Color(255, 255, 255, 235)
	--elseif 0.6 < iPercentage then
	--	fHealth = Color(146, 142, 22, 235)
	--elseif 0.3 < iPercentage then
	--	fHealth = Color(166, 79, 3, 235)
	--else
	--	fHealth = Color(153, 7, 4,math.sin(RealTime() * 6) * 127.5 + 127.5 )
--	end
...
function hud.DrawStatsPanel()
	...
	local StatsX,StatsY = ScaleW(32),34+ScaleH(790)
	...
	local ActualX = 12 + ScaleW(5) + ScaleW(12) + ScaleW(5)
	local ActualY = (ScrH()- ScaleH(73) - 12) + ScaleH(73)/2
	draw.SimpleTextOutlined(text, "NewAmmoFont21", ActualX, ActualY, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,1,Color(0,0,0,255))
	
	draw.SimpleTextOutlined("", "NewAmmoFont7", StatsX, StatsY, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))

end

function hud.box1()

--local StatsX,StatsY = ScaleW(20),ScaleH(750)
--local StatsX,StatsY = ScaleW(20),ScaleH(200)
--local ActualX = 190 + ScaleW(5) + ScaleW(12) + ScaleW(5)
--local ActualX = ScaleW(150) + ScaleW(0) + ScaleW(0)
--local ActualY = (ScrH(900)- ScaleH(900)) + ScaleH(900)
	
--draw.RoundedBox( 5, StatsX ,StatsY , ActualX , ActualY, Color(0, 0, 0, 150))
...
local text1x, text1y = WaveX+ScaleW(7), WaveY+5
	...
	if ServerTime() <= WARMUPTIME then		
		...
		if timleft < 10 then
			local glow = math.sin(RealTime() * 8) * 200 + 255
			draw.SimpleTextOutlined("Invasion starts in 0"..ToMinutesSeconds(timleft + 1), "NewAmmoFont13", ScrW() / 2, text1y, Color(255, glow, glow), TEXT_ALIGN_CENTER, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
			if lastwarntim ~= math.ceil(timleft) then
				lastwarntim = math.ceil(timleft)
				if 0 < lastwarntim then
			...
			draw.SimpleTextOutlined("Invasion starts in 0"..ToMinutesSeconds(timleft + 1), "NewAmmoFont13", ScrW() / 2, text1y, Color(255,255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
		end
	else
		local timleft = math.max(0, ROUNDTIME - ServerTime())

		draw.SimpleTextOutlined("Survive for ".. ToMinutesSeconds(timleft + 1) .." minutes", "NewAmmoFont13", ScrW() / 2, text1y, Color(255,255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
...
function hud.DrawInflictionPanel()
	...
	local text1x, text1y = ScaleW(10), WaveY+(100)
	
		
		
		
		draw.SimpleTextOutlined("Infliction:", "NewAmmoFont7", text1x, text1y, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
		
		local space1 = surface.GetTextSize("Infliction: ")
		draw.SimpleTextOutlined(cached_zombies, "NewAmmoFont7", text1x+space1, text1y, team.GetColor(TEAM_UNDEAD), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
		
		local space2 = surface.GetTextSize(cached_zombies)
		draw.SimpleTextOutlined("/", "NewAmmoFont7", text1x+space1+space2+1, text1y, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
		
		local space3 = surface.GetTextSize("/")
		draw.SimpleTextOutlined(cached_humans, "NewAmmoFont7", text1x+space1+space2+space3+2, text1y, team.GetColor(TEAM_HUMAN), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT,1, Color(0,0,0,255))
