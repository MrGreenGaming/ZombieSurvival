...
function meta:IsHumanClass()
	if self:Team() ~= TEAM_HUMAN then
		...
		commando2 = {"weapon_zs_melee_plank"}
		...
		pyrotechnic = {"weapon_zs_flaregun","weapon_zs_firebomb","weapon_zs_melee_pipe2"}
...
function meta:NextRankXP()
	...
	for i=0,self:GetRank() do
		exp = exp + XP_BLANK + XP_INCREASE_BY*(i+1) + (i*10000)
	end
...
function meta:PlaySpawnMusic()
	...
	-- Can't play dead
	if not self:Alive() or ENDROUND or LASTHUMAN or SERVER then return end
