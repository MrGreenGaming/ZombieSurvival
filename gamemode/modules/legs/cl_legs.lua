		...
		function self.LegEnt:BuildBonePositions( numbones, numphysbones )
			...
			if !LocalPlayer():InVehicle() then
				if LocalPlayer():IsHuman() then
					...
					Legs.BonesToRemove = Legs.BoneHoldTypes[ "zombies" ]
					if LocalPlayer():IsZombie() and LocalPlayer():IsHowler() then
						Legs.BonesToRemove = Legs.BoneHoldTypes[ "howler" ]
					end
	...
	if IsValid( self.LegEnt ) then
		...
		elseif LocalPlayer():IsZombie() then
			if self.Velocity > 0.5 then
				self.PlaybackRate = ( ( self.Velocity * 0.8 ) / maxseqgroundspeed )
			end
	
			self.PlaybackRate = math.Clamp(self.PlaybackRate, 0.8, 1.5)
		end
...
hook.Add( "RenderScreenspaceEffects", "Legs:Render", function() -- Need to find a better place to render. Legs half-way in water = looks like they are clipped
	cam.Start3D( EyePos(), EyeAngles() )
		if ShouldDrawLegs() then -- Render check
			...
			if LocalPlayer():IsZombie() and LocalPlayer():IsCommonZombie() then
				Legs.RenderPos.z = Legs.RenderPos.z + 11
			end
			...
			if LocalPlayer():InVehicle() then -- The player is in a vehicle, so we use the vehicles angles, not the LocalPlayer
				...
				if Legs.LeanMode:GetBool() then -- Leaning is moving the legs forward as wee look up, gives the effect of bending over
					if LocalPlayer():IsZombie() then
						Legs.ForwardOffset = -18 + ( 1 - ( math.Clamp( Legs.BiaisAngles.p - 45, 0, 45 ) / 45 ) * 7 )
					else
				...
				if LocalPlayer():GetGroundEntity() == NULL and not LocalPlayer():IsZombie() then -- Crappy duck fix
			...
			Legs.RenderColor.r, Legs.RenderColor.g, Legs.RenderColor.b, Legs.RenderColor.a = c.r,c.g,c.b,c.a -- Set color will not work in this case, so we render it manually
					...
					render.SetColorModulation( Legs.RenderColor.r / 255, Legs.RenderColor.g / 255, Legs.RenderColor.b / 255 ) -- Render the color correctly
						render.SetBlend( Legs.RenderColor.a / 255 )
							--cam.IgnoreZ( true ) --Attempted to give them draw priority over the world. Works, but they are drawn above view model ToDo: Fix
								...
								Legs.LegEnt:SetModelScale(LocalPlayer():GetModelScale(),0)
