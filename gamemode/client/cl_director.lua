			...
			if zombie:Alive() then
				if !zombie.HasBody then
					...
					Body = ClientsideModel("models/player/gordon_classic.mdl", RENDERGROUP_OPAQUE)
						...
						if !ValidEntity(Body) then return end
...
if !owner.HasBody then return end
			...
			if zombie:IsZombie() and zombie:IsValid() and not zombie:Alive() then
				if IsValid(zombie:GetRagdollEntity()) and ent == zombie:GetRagdollEntity() then
					...
					timer.Simple(0, function()
						...
						if !ValidEntity(zombie.corpse.Body) then return end
...
if not ValidEntity ( MySelf ) then return end
	for k,zombie in pairs(player.GetAll()) do
		if zombie:IsZombie() and zombie:IsValid() then
			if zombie:Alive() then
				if !zombie.HasBody then //Just a small check
				MakeMahBody()
...
/*----------------------------------------------------
          Christmas intialization - clientside
------------------------------------------------------*/
local function InitializeChristmas ()
	if not CHRISTMAS then return end
	
	//Precache gifts
	for i = 1, 4 do
		util.PrecacheModel ( "models/effects/bday_gib0"..i..".mdl" )
	end
	...
	//Precache goblin
	util.PrecacheModel ( "models/player/goblin/goblin.mdl" )
	
	//Preacache jingle songs
	util.PrecacheSound ( "sound/christmas/jingleintro.mp3" )
	util.PrecacheSound ( "sound/christmas/jinglebreak.mp3" )
		
end
hook.Add ( "Initialize", "InitChristmas", InitializeChristmas )

util.PrecacheSound ( "christmas/jinglebreak.mp3" )
local function JingleBells ( um )
	if MySelf:Team() == TEAM_SPECTATOR then return end
	RunConsoleCommand("stopsounds")

	timer.Simple ( 0.2, surface.PlaySound, "christmas/jinglebreak.mp3" ) 
	timer.Simple ( 1, function() hook.Add ( "CreateMove","JingleMove", JingleMove ) hook.Add ( "HUDPaint","JingleDraw", JingleDraw ) end ) 
end
usermessage.Hook( "JingleBells",JingleBells )

local function JingleEnd ( um )
	hook.Remove ( "CreateMove", "JingleMove" )
	hook.Remove ( "HUDPaint", "JingleDraw" )
end
usermessage.Hook ( "JingleEnd", JingleEnd )

function JingleMove( cmd )
	if not ValidEntity ( MySelf ) then return end
	if MySelf:Team() == TEAM_SPECTATOR then return end
	
	//dance 
	local aAng = cmd:GetViewAngles()
	aAng.p = aAng.p + ( math.sin ( CurTime() * 3 ) * 0.3 )
	aAng.y = aAng.y + ( math.cos ( CurTime() * 3 ) * 0.15 )
	cmd:SetViewAngles( aAng )
end

function JingleDraw()
	local Text = "           I love those J-I-N-G-L-E Bells!\n          Those holiday J-I-N-G-L-E Bells!\n         Those happy J-I-N-G-L-E Bells!\n   Jingle bells, jingle bells, jingle all the way!\n  Oh, what fun it is to ride in a one horse open sleigh!"
	draw.DrawText ( Text, "ArialNine", w * 0.5, h * 0.3, Color ( 255,255,255,255 ), TEXT_ALIGN_CENTER )
end
...
local owner = um:ReadEntity()
if owner.HasBody then return end
 owner.Body = ClientsideModel("models/player/gordon_classic.mdl", RENDERGROUP_OPAQUE)
 if !ValidEntity(owner.Body) then return end
 owner.Body:SetPos(owner:GetPos())
 owner.Body:SetAngles(owner:GetAngles())
 owner.Body:SetParent(owner) 
 owner.Body:AddEffects( EF_BONEMERGE)
 owner.HasBody = true
end
--usermessage.Hook ( "MakeBody", MakeMahBody )
...
local owner = um:ReadEntity()
if !owner.HasBody then return end
TrashbinForBodies[owner]:Remove()
TrashbinForBodies[owner] = nil
owner.HasBody = false
end
usermessage.Hook ( "DeleteBody", DeletMahBody )
...
if not ValidEntity ( MySelf ) then return end
		for k,zombie in pairs(player.GetAll()) do
			if zombie:IsZombie() and zombie:IsValid() and not zombie:Alive() then
				if IsValid(zombie:GetRagdollEntity()) and ent == zombie:GetRagdollEntity() then
					if zombie.HasBody then
						DeleteBodyEnt(zombie,TrashbinForBodies[zombie])
						...
						zombie.corpse.Body:SetParent(zombie.corpse) 
						zombie.corpse.Body:AddEffects(EF_BONEMERGE)
						zombie.corpse.Body.BuildBonePositions = zombie.corpse.BuildBonePositions //apply dismemberment effects
						zombie.corpse:SetNoDraw(true)
						Trash[zombie] = zombie.corpse.Body
					end
				end
			end
		end
...
if not ValidEntity ( MySelf ) then return end
	for k,zombie in pairs(player.GetAll()) do
		if zombie:IsZombie() and zombie:IsValid() then
			if zombie:Alive() then
				if !zombie.HasBody then
					local Body = ClientsideModel("models/player/gordon_classic.mdl", RENDERGROUP_OPAQUE)
						if !ValidEntity(Body) then return end
						Body:SetPos(zombie:GetPos())
						Body:SetAngles(zombie:GetAngles())
						Body:SetParent(zombie) 
						...
						zombie.HasBody = true
						TrashbinForBodies[zombie] = Body
				end
			end
		end
	end
		for k,v in pairs(Trash) do
			if not IsValid(k:GetRagdollEntity()) and IsValid(k) then
					Trash[k]:Remove()
					Trash[k] = nil
				end
			end
...
if not ValidEntity ( MySelf ) then return end
for k,v in pairs(TrashbinForBodies) do
		TrashbinForBodies[k]:Remove()
		TrashbinForBodies[k] = nil
		k.HasBody = false
end
for k,v in pairs(Trash) do
		Trash[k]:Remove()
