...
function SWEP:SecondaryAttack()
	if self.Berserker then
		...
		if self.Owner:GetPerk("berserker_battlecharge") then
			...
			Velocity.z = math.Clamp(Velocity.z, 200,320)		
		else
			Velocity.z = math.Clamp(Velocity.z, 200,240)		
		end
		...
		if SERVER and hitent:IsValid() then
			if hitent:GetClass() == "func_breakable_surf" then
				...
				if hitent:IsPlayer() then
					...
					local Velocity = self.Owner:EyeAngles():Forward() * damage * 2.5 * (self.MeleeSize - 0.25)					
					if damage > 40 then
						Velocity.x = Velocity.x * 0.8
						Velocity.y = Velocity.y * 0.8
						Velocity.z = Velocity.z * 1.4					
						Velocity.z = math.Clamp(Velocity.z,40,160)
					end
